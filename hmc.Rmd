...
h <- function(theta, lambda) {
 -dpois(theta, lambda, log = T)
 # theta * log(lambda) - lambda - log(gamma(theta + 1))
}
h(300, 2)

dh_dtheta <- function(theta, lambda) {
  tmp <- log(lambda) - 1 - 1/(gamma(theta + 1))
  -tmp
}

hamiltonian <- function(p, theta, lambda) {
  h(theta, lambda) + 0.5 * p^2
}

leapfrog_nexthalf_p <- function(p, theta, lambda, eps = 0.01) {
  p - 0.5 * eps * dh_dtheta(theta, lambda)
}
...
hamiltonian(p = 3, theta = 1, lambda = 11)

move_one_step <- function(theta, lambda, p, eps = 0.01, L = 100, stlide = 1) {
  res <- c(1, p, theta, hamiltonian(p, theta, lambda))
  res2 <- NULL
  for (i in 1:L) {
    p <- leapfrog_nexthalf_p(p, theta, lambda, eps)
...
h_dat <- tibble(h = pot_eng(theta, lambda = lambda), theta)
...
system.time(saveGIF(
       ...
       ani.width = 480,
       ani.height = 480,
       movie.name = "images/Hplt3.gif"
        ))

```


![gif3](images/Hplt3.gif)
